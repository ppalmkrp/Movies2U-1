...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-4">
                @foreach ($movie as $m)
                    <img src="{{ asset('Materials/Movies/' . $m->movie_id . '.png') }}" alt="Movie poster"
                    ...
                    <form method="post" action="/review" enctype="multipart/form-data">
                        ...
                        <div class="input-group mb-3 mt-3">
                            <div class="input-group mb-3">
                                ...
                                <input name="movie" value="{{ $m->movie_id }}" class="form-control"
                ...
                @else
                    <form method="post" action="/review" enctype="multipart/form-data">
                        ...
                        <div class="input-group mb-3 mt-3">
                            <div class="input-group mb-3">
                                <span class="input-group-text" id="basic-addon1">Movie:</span>
                                <input name="movie" value="{{ $m->movie_id }}" type="text" class="form-control hide"
                            ...
                            @if ($rv->user_id == $u->id && $rv->movie_id == $m->movie_id && Auth::user()->roles == 1)
                                <div class="card mt-3">
                                    <div class="card-body">
                                        <blockquote class="blockquote mb-0">
                                            <p>{{ $rv->review_info }}</p>
                                            <footer class="blockquote-footer">{{ $u->name }} <cite>
                                                    @if (Auth::user()->id == $rv->user_id)
                                                        <a href="/delcomment/{{ $rv->id }}"><button type="button"
        ...
        @elseif(Auth::user()->roles == 2 && $rv->user_id == $u->id && $rv->movie_id == $m->movie_id)
            <div class="card mt-3">
                <div class="card-body">
                    <blockquote class="blockquote mb-0">
                        <p>{{ $rv->review_info }}</p>
                        <footer class="blockquote-footer">{{ $u->name }} <cite><a
                                    href="/delcomment/{{ $rv->id }}"><button type="button"
                                        class="btn btn-outline-danger">Delete</button></a></cite></footer>
                    </blockquote>
                </div>
            </div>
            @endif
