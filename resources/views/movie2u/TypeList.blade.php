            ...
            <div class="col-10">
                <div class="row"> <!-- วนลูปนำหนังในหัวข้อที่เลือกมาโชว์ทั้งหมด -->
                    @foreach ($mtype as $mt)
                                ...
                                <div class="col-3">
                                    <div class="card mt-4" style="width: auto">
                                        <a href="/moviedetail/{{ $m->movie_id }}">
                                            <img class="card-img-type"
                                                src="{{ asset('Materials/Movies/' . $m->movie_id . '.png') }}"
                                                alt="Movie poster" width="150px" height="350px" />
                                                <a href="/addfav/{{ $mt->movie_id }}" class="btn btn-link"><i class="bi bi-heart text-danger"></i>
                                            </a>
                                            <div class="card-body">
                                                <h5 class="card-title">
                                                    {{ $m->movie_name }}
                                                ...
                                                <div class="mt-3">
                                                    ...
                                                    @else
                                                        @if (Auth::user()->roles == 1)
                                                            <a href="/moviedetail/{{ $m->movie_id }}" class="btn btn-warning"
                                                                style="width: 100%;">Detail</a>
                                                            <a href="/addwatchlist/{{ $m->movie_id }}"
                                                                class="btn btn-dark mt-2" style="width: 100%;"><i
                                                                    class="bi bi-plus-lg"></i> Watchlist</a>
                        ...
                        @endforeach
                        @if (!$foundMovies)
                            <!-- หากเลือกประเภทหนังแล้วไม่มีข้อมูลในนั้น -->
